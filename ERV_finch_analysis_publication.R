...
source("geom_boxplot_transparent.R")
...
## Problem with Tip labels
`%not_in%` <- purrr::negate(`%in%`)

# p <-
ggplot(erv_hits.summary %>% 
         mutate(erv.assigned = factor(erv.assigned, levels = erv_hits.summary %>% 
                                        arrange(-erv.insertions) %>% 
                                        pull(erv.assigned)))) +
  geom_point(aes(x = erv.assigned, y = erv.insertions)) +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())

# pdf(file = "plots/211014_insertions_per_ERV.pdf", height = 8, width = 8)
# p
# dev.off()

erv_hits.summary %>% pull(erv.insertions) %>% sum() # 425,296 -> 547,978 with Dell and CNVnator

# write_csv(erv_hits.summary, file = "211018_erv_insertions_summary.csv")

top10ervs <- erv_hits.summary %>% 
  filter(erv.insertions >= 10500) %>% #Arbitrary cutoff to select top 10 ERVs
  arrange(-erv.insertions) %>% 
...
p <-
  ...
  # geom_boxplot(aes(x = species, y = loci_per_species_sample)) +
  # geom_boxplot(aes(x = species, y = loci_per_species_sample/total_insertions)) +
  # geom_boxplot(data = coverage.df, aes(x = species, y = cov)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),text = element_text(size=6)) +
  theme(text = element_text(size = 6))

# pdf(file = "plots/211018_ERV_insertionBias_boxplot_raw.pdf", height = 4.75, width = 4.75)
...
locus_freq.df <- sample_locus_ervAssigned.df %>%
  group_by(species, erv.assigned, locus) %>% 
  summarise(locus_freq = sum(count/n())) %>% 
  mutate(locus_present = as.integer(if_else(locus_freq > 0, 1, 0)))
